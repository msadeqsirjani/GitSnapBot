...
def format_unfollow(login: str, total: int) -> Alert:
    return alert_card(
        ...
        "➖",
        "Unfollowed",
        body=f"{user_link(login)} unfollowed you",
        fields=[("Followers", str(total))],
        url=f"https://github.com/{login}",
        actor=login,
        group="unfollow",
...
def format_deleted(login: str, total: int) -> Alert:
    return alert_card(
        ...
        "👻",
        "Account deleted",
        body=f"{user_link(login)} disappeared from GitHub (404)",
        fields=[("Followers", str(total))],
        url=f"https://github.com/{login}",
        actor=login,
        group="deleted",
...
def format_unstar(login: str, repo: str, total: int | None = None) -> Alert:
    ...
    return alert_card(
        ...
        "💔",
        "Unstarred",
        body=f"{user_link(login)} unstarred {repo_link(repo)}",
        fields=fields,
        url=f"https://github.com/{repo}",
        actor=login,
        group=f"unstar:{repo}",
...
def format_star_count_delta(repo: str, delta: int, total: int) -> Alert:
    if delta > 0:
        ...
        kind, icon, title = "unstar", "💔", f"{abs(delta)} unstar{'s' if delta != -1 else ''}"
    return alert_card(
        kind,
        icon,
        title,
...
def format_fork_count_delta(repo: str, delta: int, total: int) -> Alert:
    if delta > 0:
        ...
        kind, icon, title = "unfork", "🗑", f"{abs(delta)} fork{'s' if delta != -1 else ''} deleted"
    return alert_card(
        kind,
        icon,
        title,
...
def format_traffic(repo: str, metric: str, date: str, count: int, uniques: int) -> Alert:
    if metric == "views":
        icon, label = "👁", "Repo views"
    else:
        icon, label = "📥", "Repo clones"
    return alert_card(
        f"traffic_{metric}",
        icon,
        label,
...
def format_welcome(
    ...
    rich = (
        f"<h1>🚀 {escape(bot)} is live</h1>"
        f"<p>Tracking {user_link(username)}. Activity is collected all week "
        f"and sent as one weekly report.</p>"
        ...
        f"</table>"
        f"<ul>"
        f"<li>Tap <b>📬 Report</b> anytime to preview the weekly digest</li>"
        f"<li>Use <b>Status</b> for queue size and the next send time</li>"
        f"</ul>"
        f"<details><summary>What GitHub cannot report</summary>"
        f"<p>GitHub does not expose profile page views. Repository traffic "
        f"(views and clones) is reported instead.</p></details>"
    )
    fallback = (
        f"🚀 <b>{escape(bot)} is live</b>\n\n"
        f"Tracking {user_link(username)}\n"
        f"Followers  ·  {followers}\n"
        f"Repositories  ·  {repos}\n"
        f"Stars  ·  {stars}\n\n"
        f"Activity is collected all week and sent as one weekly report.\n"
        f"Tap 📬 Report anytime to preview.\n\n"
        f"<i>GitHub does not expose profile page views. Repo traffic is reported instead.</i>"
    )
...
def format_status(
    ...
    return alert_card(
        ...
        "📊",
        "Status",
        body=f"Tracking {user_link(username)}",
        fields=[
            ("State", state),
            ("Followers", str(followers)),
            ("Repos", str(repos)),
            ("Queued for report", str(queued)),
            ("Next report", escape(next_digest or "n/a")),
            ("Reports sent", str(sent)),
            ("API remaining", escape(rate)),
            ("Last poll", escape(last_poll or "n/a")),
        ],
...
def format_notification(notification: dict[str, Any]) -> Alert | None:
    ...
    label = NOTIFICATION_REASONS.get(reason, reason.replace("_", " ") or "notified you")
    if label is None:
        return None
...
def _format_issue_comment(event: dict[str, Any]) -> Alert | None:
    ...
    return alert_card(
        ...
        "💬",
        "Comment",
        body=f"{user_link(actor)} on {repo_link(repo)}#{number}",
...
def _format_release(event: dict[str, Any]) -> Alert:
    ...
    return alert_card(
        "release",
        "🏷",
        f"Release {payload.get('action') or 'published'}",
        body=f"{user_link(actor)} on {repo_link(repo)}",
        quote=escape(name),
...
def _format_discussion(event: dict[str, Any]) -> Alert:
    ...
    return alert_card(
        "discussion",
        "💭",
        f"Discussion {payload.get('action') or 'updated'}",
        body=f"{user_link(actor)} on {repo_link(repo)}",
...
def compact_alerts(alerts: list[Alert], threshold: int = 4) -> list[Alert]:
    ...
    for key in order:
        ...
        extra = f" and {len(unique_actors) - 8} others" if len(unique_actors) > 8 else ""
        kind = bucket[0].kind
        titles = {
        ...
        icon, title = titles.get(kind, ("📌", f"{kind} events"))
        compacted.append(
            alert_card(
                kind,
                icon,
